    ...
    if i >= Array.length expr then ()
      ...
      | Ir3.Destruct -> (
          ...
          | Values.Tuple vals ->
              ...
              List.iter (fun v -> Stack.push v vs) vals';
              helper expr (i + 1) scope
          | _ -> assert false (* function *))
      | Ir3.FnVal (c, symcnt, len) ->
          let c' = Array.map (fun name -> scope.(name)) c in
          Stack.push (Values.Fn (c', symcnt, i + 1)) vs;
          helper expr (i + 1 + len) scope
      | Ir3.FnCall -> (
          let arg = Stack.pop vs in
          let fn = Stack.pop vs in
          match fn with
          | Values.Fn (closure, symcnt, loc) ->
              let scope' = Array.make symcnt Values.Nil in
              scope'.(0) <- fn;
              Array.iteri (fun i v -> scope'.(i + 1) <- v) closure;
              Stack.push arg vs;
              Stack.push scope cs;
              Stack.push (i + 1) ls;
